...
shinyServer(function(input, output) {
  ...
  syntax.highlighted <- reactive({
    ...
    txt <- c('<pre><span style="font-family: monospace">',txt,
              '\nFormatted By <a href="http://www.econometricsbysimulation.com">Econometrics by Simulation</a>',
              '</span></pre>')
    writeClipboard(paste(txt, collapse="\n"))
    txt
  })
  
  output$formatted <- renderPrint({cat(paste(syntax.highlighted(), collapse="\n"))})
    
  output$htmlformatted <- renderPrint({
